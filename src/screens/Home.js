        ...
        return (
            <View key={index}>
                <View row={true} alignItems='center' py={rv(hp(3))}>
                    <TouchableOpacity onPress={() => navigation.navigate('Details', { title: item.codigo, type: item.unidad_medida })} style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <View w={'85%'} ml={rv(hp(1.5))}>
                            <Text fontSize={rv(hp(2.8))} my={3} fontWeight='bold' color='blue700'>{item.codigo}</Text>
